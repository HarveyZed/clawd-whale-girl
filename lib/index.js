...
/**
 * Version shown in the restart banner. Read from package.json instead of being
 * pinned here: the literal used to be a second source of truth and silently kept
 * reporting the previous release after a bump (the banner is exactly what you
 * read to tell which build is running). Bumping the manifest is now the only step.
 */
export function readPackageVersion(readFileImpl = readFileSync) {
  try {
    const parsed = JSON.parse(readFileImpl(join(PACKAGE_ROOT, 'package.json'), 'utf8'))
...
function sessionFields(session) {
  ...
  // Subagent sessions are never headless (F3). Clawd skips headless sessions
  // both when it merges the main state (state-priority.js) and when it counts
  // the juggling tier (state-visual-resolver.js countLiveSubagents), so marking
  // one headless silently kills the animation. Keeping subagent sessions out of
  // the HUD is Clawd's own presentation policy — do not reintroduce a switch
  // for it here.
  return {
...
// `opts` is retained for API compatibility only: no caller passes it, and
// nothing reads it now that the subagent-headless switch is gone.
...
export function buildApprovalPayload(req) {
  ...
  // Deliberately no `headless` flag here, not even for subagent sessions:
  // Clawd treats an explicit headless:true as "not interactive" and silently
  // falls back to the native answerer (server-route-permission.js
  // isHeadlessPermissionRequest), which would leave the pet blinking
  // `notification` (F2) with no bubble to act on. Subagent approvals are still
  // human decisions, so they bubble like any other.
  return {
    agent_id: AGENT_ID,
    hook_source: HOOK_SOURCE,
