...
async function discoverUncached(options = {}) {
  const identity = await readRemoteIdentity(options)
  if (identity) {
    const transport = { port: identity.remotePort, nonce: identity.routingNonce, source: 'ssh-remote' }
    ...
    if (await probe(transport.port, transport)) {
