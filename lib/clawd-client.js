...
/**
 * Default identity locations in priority order: ~/.claude/hooks is where the
 * SSH deploy stages clawd-remote.json next to the command hooks, and ~/.clawd
 * is this plugin's own fallback (Clawd itself only knows the first one).
 */
function defaultIdentityPaths(home = homedir()) {
  return [
    join(home, '.claude', 'hooks', REMOTE_IDENTITY_FILENAME),
    join(home, '.clawd', REMOTE_IDENTITY_FILENAME),
  ]
}

/**
 * Identity candidates. CLAWD_REMOTE_IDENTITY_PATH is exclusive: when it is set,
 * nothing else is considered — the same precedence Clawd's own
 * resolveRemoteIdentityPath applies. A set-but-unusable override must therefore
 * fail closed instead of quietly falling through to another identity.
 */
function remoteIdentityPaths(env = process.env, home = homedir()) {
  const override = env && typeof env.CLAWD_REMOTE_IDENTITY_PATH === 'string'
    ? env.CLAWD_REMOTE_IDENTITY_PATH.trim()
    : ''
  if (override) return [override]
  return defaultIdentityPaths(home)
}
...
/**
 * Secure-marker candidates: the explicit override, else one beside each default
 * identity location. Deliberately NOT derived from CLAWD_REMOTE_IDENTITY_PATH —
 * Clawd's marker path does not follow that override either, so relocating the
 * identity must not relocate the marker that proves this host is a managed
 * remote. (An explicit `paths` injection, used by tests, still wins.)
 */
function secureMarkerPaths(options = {}) {
  const env = options.env || process.env
  const override = typeof env[SECURE_MARKER_ENV] === 'string' ? env[SECURE_MARKER_ENV].trim() : ''
  if (override) return [override]
  const paths = options.paths || defaultIdentityPaths(options.home)
